# Build
docker build -t captureorder-netcore . 
...
docker run -it -p 8080:8080 --env-file env.mongo.rabbit.list --network challenge captureorder-netcore

# Run the app with Cosmos+EventHub
docker run -it -p 8080:8080 --env-file env.cosmos.eventhub.list --network challenge captureorder-netcore